...
const ProjectDetails = ({ project }) => {
  ...
  return (
    <div className="project bg-white p-5 rounded-md shadow-md border border-stone-300 flex flex-col gap-5 w-[30rem]">
      <div className="top">
      ...
      <div
        ...
        className={`overlay fixed z-[1] h-screen w-screen bg-slate-900/50 backdrop-blur-sm top-0 left-0 bottom-0 right-0 ${
          isOverlayOpen ? "" : "hidden"
        }`}
      ></div>
      {/* modal */}
      <div
        className={`update-modal w-[35rem] fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-10 rounded-md shadow-md border-stone-200 z-[2] ${
          isModalOpen ? "" : "hidden"
        }`}
      >
